...
//a kettő canvas clickes eventlistener fel és le dobálása kondició vizsgálgatás helyett
...
class Modell {
    constructor(cols, rows) {
        this.speeds = [100, 250, 500, 750, 1000, 1500, 2000];
...
class View {
    ...
    canvasSelectClick(canvas, e) {
        ...
        if (this.selectMode === 1) {
            ...
            const rect = canvas.getBoundingClientRect();
            const x = Math.floor(((event.clientX - rect.left) / currResolution) + this.renderStartX);
            const y = Math.floor(((event.clientY - rect.top) / currResolution) + this.renderStartY);
        ...
        //TODO Megvalósítani a selectet úgy hogy 1 es módban kijelül sárgára egy négyzetet és 2es módba lép(hozzáadhatna egy event listenert ilyenkor, ami húzásra reagal a canvas felett és jelöl)
        //2es módban a kattintás véglegesíti a kijelölt területet és 3as módba lép ahol lehet valamit kezdeni a területtel
    }
